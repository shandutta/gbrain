...
function runList(_args: string[]): void {
  const bundled = Array.from(BUNDLED_PACK_NAMES).sort();
...
function packPathByName(name: string): string | null {
  if (BUNDLED_PACK_NAMES.has(name)) {
    ...
    // core/schema-pack/load-active.ts so inspection verbs can see every
    // bundled pack that the active-pack resolver can load.
