...
export class PostgresEngine implements BrainEngine {
  ...
  async getHealth(): Promise<BrainHealth> {
    ...
    const nibDomains = await sql`
      ...
      LIMIT 8
    `;
    const noInbound = Number(noInboundRow?.no_inbound ?? 0);
    const totalLinkable = Number(noInboundRow?.total_linkable ?? 0);
    const nibRatio = totalLinkable > 0 ? noInbound / totalLinkable : 0;
    const nibTopDomains = (nibDomains as unknown as { domain: string; cnt: number }[]).map(
